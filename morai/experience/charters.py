...
def pdp_polars(
    ...
    """
    Partial Dependence Plot optimized with Polars and GPU predictions.
    """

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_device = (
        next(model.parameters()).device if hasattr(model, "parameters") else device
    )
    ...
    # Feature processing
    X_cols = model.get_features() if hasattr(model, "get_features") else df.columns
    X = df.select(X_cols)
    ...
    if mapping and x_axis in mapping:
        ...
        x_axis_values = df[x_axis].unique().to_list()
        if df[x_axis].dtype in [pl.Int64, pl.Float64]:
            x_axis_values = sorted(x_axis_values)

    # line_color values
    if line_color:
        line_values = df[line_color].unique().to_list()
    else:
        df = df.with_columns(pl.lit("Overall").alias("line_color"))
        line_color = "line_color"
        line_values = ["Overall"]

    results = []

    # Batch iteration for GPU efficiency
    for x_val, line_val in tqdm(
        product(x_axis_values, line_values), desc="PDP processing"
    ):
        batch_df = df.clone()

        # Set x_axis and line_color for the current combination
        batch_df = batch_df.with_columns(
            [pl.lit(x_val).alias(x_axis), pl.lit(line_val).alias(line_color)]
        )

        # Convert to torch tensors
        X_num, X_cat_idx = model._prepare_input_tensor(batch_df.to_pandas())
        X_num = X_num.to(device)
        X_cat_idx = [idx.to(device) for idx in X_cat_idx]

        # Prediction
        with torch.no_grad():
            pred = model(X_num, X_cat_idx)
            if weights is not None:
                pred = pred * torch.tensor(weights, device=device, dtype=torch.float32)
            mean_pred = pred.mean().item()

        results.append({x_axis: x_val, line_color: line_val, "pred": mean_pred})

    # Convert results to Polars DataFrame
    pdp_df = pl.DataFrame(results)

    # Centering
    if center == "global":
        global_mean = pdp_df["pred"].mean()
        pdp_df = pdp_df.with_columns(
            ((pdp_df["pred"] - global_mean) / global_mean + 1).alias("%_diff")
        )
    elif center == "raw":
        pdp_df = pdp_df.with_columns(pdp_df["pred"].alias("%_diff"))
    else:
        raise NotImplementedError("per_x centering not yet implemented for Polars")

    # Create plotly figure
    fig = make_subplots(rows=2 if secondary else 1, cols=1)
    colorscale = px.colors.qualitative.Light24
    for i, lv in enumerate(pdp_df[line_color].unique().to_list()):
        subset = pdp_df.filter(pl.col(line_color) == lv)
        fig.add_trace(
            go.Scatter(
                x=subset[x_axis].to_list(),
                y=subset["%_diff"].to_list(),
                mode="lines",
                name=lv,
                line={"color": colorscale[i % len(colorscale)]},
            ),
            row=1,
            col=1,
        )
    ...
    fig.update_layout(
        ...
        yaxis_title=f"%_diff ({center})",
        height=400 * (2 if secondary else 1),
        width=800,
    )

    return fig if display else pdp_df
