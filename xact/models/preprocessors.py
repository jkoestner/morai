...
def preprocess_data(
    ...
    X = model_data[passthrough_cols + ordinal_cols + nominal_cols + ohe_cols].copy()
    ...
    X[ordinal_cols] = ordinal_encoder.fit_transform(X[ordinal_cols]).astype("int32")
    ...
    X = pd.get_dummies(X, columns=ohe_cols, dtype=int)
    for col in ohe_cols:
        mapping[col] = {
